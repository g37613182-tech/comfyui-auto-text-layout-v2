...
class AutoTextLayout:
    """
    ComfyUI node v2: Multi-factor scoring layout engine.
    Analyzes composition via subject mask, generates candidate regions,
    scores each by 6 aesthetic/readability rules, picks the optimal one.
    """
    ...
    def INPUT_TYPES(cls):
        return {
            ...
            "optional": {
                ...
                # --- scoring weights (exposed for fine-tuning) ---
                ...
                "min_ratio": ("FLOAT", {"default": 2.0, "min": 1.0, "max": 5.0, "step": 0.1,
                                         "tooltip": "Hard minimum width/height ratio for text block. "
                                                     "Candidates below this are eliminated."}),
                "min_chars_per_line_en": ("INT", {"default": 15, "min": 5, "max": 50, "step": 1,
                                                   "tooltip": "Minimum chars per line (English) before elimination"}),
                "min_chars_per_line_cjk": ("INT", {"default": 6, "min": 2, "max": 20, "step": 1,
                                                    "tooltip": "Minimum chars per line (CJK) before elimination"}),
                "show_debug": ("BOOLEAN", {"default": False,
                                            "tooltip": "Output debug image showing all candidates and scores"}),
            }
        }
    ...
    DESCRIPTION = ("v2: Multi-factor scoring layout. Analyzes composition balance, "
                   "text readability, breathing room, and aesthetic position to find "
                   "the optimal text placement.")
    ...
    def _build_avail_map(self, mask_np, margin, subject_padding):
        """Build binary availability map: 1 = safe for text, 0 = blocked."""
        avail = (1.0 - mask_np > 0.5).astype(np.uint8)

        # Erode around subject
    ...
    def _subject_centroid(mask_np, W, H):
        """Compute (cx, cy) of the subject from mask (1=subject)."""
    ...
    def _generate_candidates(self, avail, W, H, margin):
        """
        Generate diverse candidate rectangles across the image.
        Strategy: grid-based anchors × multiple aspect ratios × scale levels.
        """
        candidates = []

        # Grid anchors: 5×5 grid of center points
        grid_nx, grid_ny = 5, 5
        # Aspect ratios to try (width:height)
        ratios = [2.0, 2.5, 3.0, 4.0, 5.0, 6.0]
        # Scale: fraction of image area the candidate covers
        ...
        for gi in range(grid_nx):
            for gj in range(grid_ny):
                ...
                for ratio in ratios:
                    for scale in scales:
                        area = W * H * scale
                        # w * h = area, w / h = ratio → w = sqrt(area * ratio), h = sqrt(area / ratio)
                        ...
                        # Quick overlap check: what fraction of this rect is available?
                        crop = avail[y:y + ch, x:x + cw]
                        if crop.size == 0:
                            continue
                        avail_frac = np.mean(crop)
                        if avail_frac < 0.80:  # need >=80% clear
                            continue
        ...
        # Also add the largest-rectangle result as a candidate (legacy fallback)
    ...
    def _score_candidates(self, candidates, mask_np, avail, W, H,
                          ...
                          text, font, font_size, avg_char_w, is_cjk,
                          line_spacing, min_ratio, min_cpl, weights):
        """Score each candidate with 6 rules. Returns list of {region, score, scores_detail}."""
        ...
        for (rx, ry, rw, rh) in candidates:
            ...
            # ---- Rule 3: Readability (chars per line) ---- #
            cpl = rw / max(avg_char_w, 1)
            if cpl < min_cpl:
                continue  # hard elimination
            target_cpl = min_cpl * 2.5  # ideal: 2.5× the minimum
            ...
            # Normalize by weight sum (in case user weights don't sum to 1)
    ...
    def _render_text(self, pil_img, text_mask_img, text, font, font_size,
        ...
        """Render text into region. Returns (text_y_start, text_w, text_h)."""
        rx, ry, rw, rh = region
    ...
    def _draw_debug(self, img_np, all_candidates, scored, best, subj_cx, subj_cy, W, H):
        """Draw all candidates, scores, and the chosen region on the image."""
        debug = Image.fromarray(img_np, 'RGB').copy()
        draw = ImageDraw.Draw(debug)

        # Draw subject centroid
        r = 8
        draw.ellipse([subj_cx - r, subj_cy - r, subj_cx + r, subj_cy + r], fill=(255, 0, 0))

        # Draw ideal counter-balance point
        ...
        for i in range(1, 3):
            draw.line([(W * i // 3, 0), (W * i // 3, H)], fill=(255, 255, 0, 80), width=1)
            draw.line([(0, H * i // 3), (W, H * i // 3)], fill=(255, 255, 0, 80), width=1)

        # Draw all scored candidates (gray)
        for s in scored:
            rx, ry, rw, rh = s["region"]
            score = s["score"]
            # Color: gray → green by score
            g = int(score * 255)
            draw.rectangle([rx, ry, rx + rw, ry + rh], outline=(100, g, 100), width=1)

        # Draw best (bright green thick)
        if best:
            ...
            if "detail" in best:
                ...
                for k, v in best["detail"].items():
                    txt = f"{k}: {v:.2f}"
                    draw.text((bx, label_y), txt, font=label_font, fill=(0, 255, 0))
                    label_y += 16
    ...
    def _wrap_text(text, font, max_width):
        """Character-level wrapping (CJK-safe), respects explicit newlines."""
        if max_width <= 0:
            return [text]
    ...
    def _calc_auto_font_size(self, text, font_path, region, line_spacing):
        """Binary search for font size filling ~50% of region."""
        _, _, rw, rh = region
    ...
    def _resolve_alignment(alignment, rx, rw, W):
        """Resolve 'auto' alignment based on region position."""
        if alignment != "auto":
            return alignment
        region_cx = rx + rw / 2
        img_cx = W / 2
    ...
    # ================================================================== #
    #                     LARGEST BLANK RECT (legacy fallback)            #
    # ================================================================== #

    def _largest_blank_rect(self, avail, W, H, margin):
        """Histogram-based maximal rectangle. Used as one candidate among many."""
